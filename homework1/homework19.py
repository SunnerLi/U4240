...
if __name__ == "__main__":
    ...
    for i in range(trainingTimes):
        X, Y = shuffle(X, Y)
        #error = pocket(X, Y, XTest, YTest, skipValid=True)
        error = pocket2(X, Y, XTest, YTest, skipValid=True)
        #print "Epoch: ", i, "\tError: ", error
        errors.append(error)
    print "Error Rate: ", float( np.sum(np.array(errors)) / trainingTimes ) 
    pocketDraw("Error Rate(update time: 50, without pocket weight)", errors)